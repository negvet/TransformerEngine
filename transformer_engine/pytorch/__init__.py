...
_PYTHON_ONLY = bool(int(os.getenv("NVTE_PYTHON_ONLY", "0")))


@functools.lru_cache(maxsize=None)
def torch_version() -> tuple[int, ...]:
    """Get PyTorch version without importing bindings.

    In python-only mode, returns (0, 0, 0) to indicate torch is not loaded.
    Otherwise, imports torch and parses its version.
    """
    if _PYTHON_ONLY:
        return (0, 0, 0)
    import torch as _torch
    return PkgVersion(str(_torch.__version__)).release


def _initialize_torch_bindings() -> None:
    """Initialize Transformer Engine PyTorch bindings and public API."""
    if _PYTHON_ONLY:
        return
